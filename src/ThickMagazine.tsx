...
import { useGLTF, useAnimations, useTexture, Gltf } from "@react-three/drei";
...
export default function ThickMagazine(props: JSX.IntrinsicElements["group"] & ThickMagazineProps) {
  const group = useRef<THREE.Group>(null);
  const actionRef = useRef<THREE.AnimationAction>();
  const keyframeRef = useRef(0);
  let { nodes, _materials, animations } = useGLTF("/ThickMagazine.glb") as GLTFResult;
  const { ref, actions } = useAnimations(animations);
  const defaultMaterial = new THREE.MeshStandardMaterial({ color: "white" });
  const pageMaterials = new Array<THREE.MeshStandardMaterial>(5).fill(defaultMaterial);
